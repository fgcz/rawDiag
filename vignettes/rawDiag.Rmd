...
```{r rawDiagLogo, echo=FALSE, out.width="50%", fig.cap="The octopussy `rawDiag` package logo by Lilly van de Venn."}
knitr::include_graphics("octopussy.png")
```
# Introduction

Over the past decade, liquid chromatography coupled to mass 
...
platform-independent software tool implemented in R language that 
...
if (isFALSE(rawrr::.checkDllInMonoPath())){
   message ("exec", "rawrr::installRawFileReaderDLLs()")
}

if (isFALSE(file.exists(rawrr:::.rawrrAssembly()))){
   message("exec", "rawrr::installRawrrExe()")
}
...
`r BiocStyle::Biocpkg('ExperimentHub')`'s
`r BiocStyle::Biocpkg('tartare')` package.
...
read the instrument raw data by using the `r BiocStyle::Biocpkg('rawrr')` package.
...
query and list all plot methods of the `r BiocStyle::Biocpkg('rawDiag')` package.
...
for more information on the plot methods, please read the package man pages and
the application examples in manuscript [@Trachsel2018].

## On using Spectra and MsBackendRawFileReaderpackage

```{r backendInitialize, eval = FALSE}
...
```{r backendQuery, eval = FALSE}
as.rawDiag.Spectra <- function(x){
  ...
  df <- data.frame(
    scan = Spectra::scanIndex(x),
    scanType = Spectra::scanType(x),
    StartTime = Spectra::rtime(x),
    precursorMass = Spectra::precursorMz(x),
    MSOrder = Spectra::msLevel(x),
    charge = Spectra::precursorCharge(x),
    Spectra::collisionEnergy(x)
  )
  
  
  df$MSOrder[df$MSOrder == 1] <- "Ms" 
  df$MSOrder[df$MSOrder == 2] <- "Ms2" 
