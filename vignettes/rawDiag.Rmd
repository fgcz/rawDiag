...
`r BiocStyle::Biocpkg('rawDiag')` proides a wrapper function `readRaw` using the `r BiocStyle::Biocpkg('rawrr')` methods
`raw::readIndex`, `rawrr::readTrailer`, and `rawrr::readChromatogram` to read
proprietary mass spectrometer generated data by 
invoking third-party managed methods through a `system2` `text connection`. 
See the stack below:
...
```{r install, echo=TRUE}
if (isFALSE(require("BiocManager", quietly = TRUE)))
    install.packages("BiocManager")

if (isFALSE(require("rawrr", quietly = TRUE)))
