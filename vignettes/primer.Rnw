...
the data for a single injection per method.
...
Assume we have a generic starting method which we want to
...
respectively. After the data has been recorded with these two new methods
we can visualize and analyse the results with the dignostic
...
signal response and chromatography is ok for the three injections as they are almost identical. As next point,
we want to check the actual cycle time
...
Here we move grey box indicating the faceting labels from the standard position on the right side 
...
In Figure~\ref{fig:cycle} we can see the the cycle time for the top 18 method (file: 09\_S174020) is
...
$\approx$10-15 MS1 points for the top 72 method. This is a better value
compared to the initial method since the instrument is spending more time to generate fragment
spectra which should ultimately lead to more peptide and protein identifications.
...
<<label=PlotCycleLoad, fig = TRUE, echo=TRUE, include=FALSE, warnings=TRUE>>=
print(gp <- PlotCycleLoad(df, method = "trellis"))
@

\begin{figure}
\includegraphics[width=0.89\columnwidth]{primer-PlotCycleLoad}

\caption{Cycle load plot -- The plot shows the injection time density of each mass spectrometry file as a violin plot. The higher the maximum number of MS2 scans is in the method, the more the density is shifted towards the maximum injection time value.
}
...
From the plot, we can see that all mehtods actually use the maximum available number of MS2 scans during the main peptide elution phase. The blue line is representing the trend as a fitted gam model.

<<label=PlotInjectionTime, fig = TRUE, echo=TRUE, include=FALSE, warnings=TRUE>>=
...
\begin{figure}
\includegraphics[width=0.89\columnwidth]{primer-PlotInjectionTime}
...
In order to check, if the instrument is able to collect an appropriate amount of ions for all MS2 scans, we can check the injetion time characteristics for the three runs. From figure~\ref{figure:IT} one can see that the injection time distribution is shiftet towards longer injection times, the more MS2 scans are available. This could could lead to issues with the spectra quality and can be checked by performing a data base search and compare the assignment rate for the three runs. If indeed an issue can be detected, this finding would then be the starting point for the next iteration cycle where we could modify the maximum injection time parameter in order to check if we can further optimize the method.
